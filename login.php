    ...
    if (mysqli_num_rows($result) == 1) {
        while ($row = mysqli_fetch_assoc($result)) {
            if ($row['enabled']) {
                ...
                echo '<script>toastr.error("Your account has not been approved yet.")</script>';
            }
            break;
        }
    } else {
...
<div class="w-screen h-screen grid place-content-center bg-gray-100">
    <form action="login.php" method="POST" class=" w-[500px] p-6 flex flex-col gap-4 shadow-lg bg-white rounded-md border">
        ...
        <div class="mt-3">
            ...
            <input type="email" name="email" class="border w-full text-base px-6 py-4 rounded-md focus:outline-none focus:border-gray-600" placeholder="Email" required />
        </div>
        <div class="mt-3">
            <label for="password" class="block text-base mb-2">Password</label>
            <input type="password" name="password" class="border w-full text-base px-6 py-4 rounded-md focus:outline-none focus:border-gray-600" placeholder="Password" required />
        </div>
        <div class="mt-5">
            <input type="submit" name="login" class="border-2 border-indigo-600 bg-indigo-600 cursor-pointer text-white py-3  w-full rounded-md hover:bg-transparent hover:text-indigo-700 font-semibold" value="Submit" />
            <hr>
